...
for (f in unique(temp$Formulation)) {
  for (r in unique(temp$Realization)) {
    temp2 = temp[which(temp$Realization == r & temp$Formulation == f),]
    if (length(unique(temp2$Utility)) == 1) {
      print(r); print(f)
      missing_utility = c("Chatham", "Pittsboro")[which(c("Chatham", "Pittsboro") != unique(temp2$Utility))]
      temp3 = temp2[1,]; temp3$Utility = missing_utility
      PWS = rbind(PWS, temp3)
    }
  }
}
...
# add OWASA dummy names to show that nothing gets triggered - single realization triggers
# should be light enough color so as to not show up visually
PWS = rbind(PWS, data.frame(Realization = rep(0, 3),
                            Utility = rep("OWASA", 3),
                            Week = rep(1, 3),
                            Project = c("Cane Creek Reservoir\nExpansion", "Stone Quarry\nExpansion", "University Lake\nExpansion"),
                            Solution = rep(16795, 3), 
                            Formulation = rep("1) Fixed Capacity", 3),
                            Set = rep("All Realizations", 3)))


E = ggplot() +
  geom_tile(data = PWS, aes(y = Project, x = as.numeric(as.character(Week))/52 + 2015), 
            color = NA, fill = "black",
...
#  scale_fill_manual(values = f_colors) +
  guides(fill = FALSE) +
  ylab("Potential Infrastructure Project") + xlab("Year") + xlim(c(2019, 2061))
...
ggsave(plot = E, paste("results_figures/E_new_update.png", sep = ""), 
