...
OM_base$Satisficing = "Satisficing\nSolutions"
...
ggsave(plot = A_base_table, "results_figures/D1_base.png", dpi = 600, height = 3.5, width = 7, units = "in")
...
ggsave(plot = A_base_set_table, paste("results_figures/D1_final.png", sep = ""), dpi = 600, height = 3.5, width = 7, units = "in")
...
select_realizations = read.csv("demand_bootstraps/bootstrap_realizations_2_100_S1.csv", header = FALSE)
high_real = select_realizations[1,1] # also tested 142, 129, 126, 117, 116, 113
...
SF1 = Satisficing[which(Satisficing$group == 1),c(1:6,42,43,59,60)]
SF2 = Satisficing[which(Satisficing$group == 2),c(1:6,42,43,59,60)]
partner_utility_ids = c(0,1,4,5)
r_set = c("High Demand", "Low Demand"); forms = c("1) Fixed Capacity", "2) Adjustible Capacity")
...
AMSY$value[which(AMSY$Week < WJLWTP_Low_HighDemand_BuildWeek_Variable &
                   AMSY$Variable == "WJLWTP Low Allocation" &
                   AMSY$Formulation == "2) Adjustible Capacity" &
                   AMSY$Realization == "High Demand")] = 0
...
D = ggplot(data = AMSYS) + 
  ...
  scale_fill_manual(values = f_colors) + xlab("Year") + ylab("Western Jordan Lake WTP Treatment Capacity Allocation (MGD)") +
  theme(strip.background = element_blank(), strip.text = element_text(size = 13, face = "bold"),
...
ggsave(paste("results_figures/D.png", sep = ""), 
       dpi = 600, height = 5, width = 7, units = "in")
...
AMSYSDF$DebtService[which(AMSYSDF$Week > (WJLWTP_Low_LowDemand_BuildWeek_Variable+52*25) & 
                            AMSYSDF$Realization == "Low Demand" & AMSYSDF$Formulation == "2) Adjustible Capacity")] = 0
  ...
  for (r in unique(DSsums$Realization)) {
    ...
    if (s == "1) Fixed Capacity" & r == "High Demand") {
      D_set = D_set + annotation_custom2(grob = textGrob("Total Debt\nService\nShares", gp = gpar(fontsize = 7)),  
                                         xmin = 2018, xmax = 2018, 
                                         ymin = 26, ymax = 26, data = DSSR)
    }
  }
}
D_final = ggplot_gtable(ggplot_build(D_set))
D_final$layout$clip[D_final$layout$name == "panel"] = "off"
ggsave(plot = D_final, "results_figures/D_final.png", dpi = 1200, units = "in", height = 5, width = 7)
