...
write.table(Pareto[,8:77], "Reference_DVs_mixedNFE.csv", sep = ",", row.names = FALSE, col.names = FALSE)
...
write.table(Satisficing[,8:77], "Satisficing_DVs_mixedNFE.csv", sep = ",", row.names = FALSE, col.names = FALSE)
...
ggsave(plot = A_base_table, "results_figures/A_base_new_nolegend.png", dpi = 1200, height = 3.5, width = 5.5, units = "in")


# steps for animation
OMS = OM[which(OM$Satisficing != "Full Reference Set"),]
f_colors_list = list(c(wes_palette(n = 1, name = "FantasticFox1"), "grey90", "grey70"),
                     c(wes_palette(n = 2, name = "FantasticFox1")[2], "grey90", "grey70"),
                     c(wes_palette(n = 3, name = "FantasticFox1")[3], "grey90", "grey70"))
...
for (f in unique(OMS$Formulation)[c(2,3,1)]) {
  ...
  ggsave(plot = A_base_set_table, paste("results_figures/A_", f, "_update_nolegend.png", sep = ""), 
