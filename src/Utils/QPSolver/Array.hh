...
template <typename T>
inline Vector<double> rankk(const Vector<T>& v)
{
    Vector<T> tmp(v);
    Vector<double> tmp_rankk(0.0, v.size());
    ...
    for (unsigned int i = 0; i < tmp.size(); i++)
    {
        ...
        for (unsigned int j = 0; j < tmp.size(); j++)
            ...
            if (tmp[j] < tmp[i])
                ...
                equal++;
        tmp_rankk[i] = smaller + 1;
        if (equal > 0)
        {
            for (unsigned int j = 1; j <= equal; j++)
                tmp_rankk[i] += smaller + 1 + j;
            tmp_rankk[i] /= (double)(equal + 1);
        }
    }
...
template <typename T>
inline Matrix<double> rankk(const Matrix<T> m)
{
    Matrix<double> tmp(m.nrows(), m.ncols());
    for (unsigned int j = 0; j < m.ncols(); j++)
        tmp.setColumn(j, rankk<T>(m.extractColumn(j)));

    return tmp;
}

template <typename T>
inline Matrix<double> r_rankk(const Matrix<T> m)
{
    Matrix<double> tmp(m.nrows(), m.ncols());
    for (unsigned int i = 0; i < m.nrows(); i++)
        tmp.setRow(i, rankk<T>(m.extractRow(i)));
