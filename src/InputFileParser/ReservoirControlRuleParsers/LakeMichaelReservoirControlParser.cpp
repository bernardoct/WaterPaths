...
LakeMichaelReservoirControlParser::parseVariables(vector<vector<string>> &block,
    ...
    for (unsigned long i = 0; i < block.size(); ++i) {
        ...
        if (line[0] == "wq_lims") {
            ...
            lillington_gage_streamflows = Utils::parse2DCsvFile(line[1],
                                                                n_realizations);
            lillington_gage_catchment = Catchment(lillington_gage_streamflows,
                                                  n_weeks);
        }
    }
...
MinEnvFlowControl *
LakeMichaelReservoirControlParser::generateReservoirControlRule(vector<vector<string>> &block,
                                                                int line_no,
                                                                int n_realizations,
                                                                int n_weeks,
                                                                const map<string, int> &ws_name_to_id,
