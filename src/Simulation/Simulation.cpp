...
void Simulation::setupSimulation(
    ...
    if (water_sources_graph.getNumber_of_vertices() != water_sources.size()) {
        __throw_invalid_argument("Number of water sources in water sources "
                                 "vector must be the same as number of "
                                 "vertices in the connectivity graph.");
    }
